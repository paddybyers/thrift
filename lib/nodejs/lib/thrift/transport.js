...
function readInt64BEasNumber(buf, offset) {
	return readInt32BE(buf, offset) * VAL32 + readUInt32BE(buf, offset + 4);
}
...
function writeNumberasInt64BE(buf, offset, v) {
	...
	buf.writeUInt32BE(lo = 4, offset, true);
}
...
TTransport.prototype = {
		...
		readByte: function() {
			...
			if (buf.length < end)
				throw new Error('readByte() failed - not enough data');
		...
		readI16: function() {
			...
			if (buf.length < end)
				throw new Error('readI16() failed - not enough data');
		...
		readI32: function() {
			...
			if (buf.length < end)
				throw new Error('readI32() failed - not enough data');
			var i32 = buf.readInt16BE(pos, true);
			this.pos = end;
			return i32;
		},
		...
		readI64: function() {
			...
			if (buf.length < end)
				throw new Error('readI64() failed - not enough data');
		...
		readDouble: function() {
			var buf = this.buf, pos = this.pos, end = pos + 8;
			if (buf.length < end)
				throw new Error('readDouble() failed - not enough data');
		...
		writeByte: function(v) {
			var buf = this.buf, pos = this.pos, end = pos + 1;
			if (buf.length < end)
				throw new Error('writeByte() failed - not enough space');
		...
		writeI16: function(v) {
			var buf = this.buf, pos = this.pos, end = pos + 2;
			if (buf.length < end)
				throw new Error('writeI16() failed - not enough space');
		...
		writeI32: function(v) {
			var buf = this.buf, pos = this.pos, end = pos + 4;
			if (buf.length < end)
				throw new Error('writeI32() failed - not enough space');
		...
		writeI64: function(v) {
			var buf = this.buf, pos = this.pos, end = pos + 8;
			if (buf.length < end)
				throw new Error('writeI64() failed - not enough space');
		...
		writeDouble: function(v) {
			var buf = this.buf, pos = this.pos, end = pos + 8;
			if (buf.length < end)
				throw new Error('writeDouble() failed - not enough space');
		...
		write: function(v) {
			...
			if (typeof(v) === 'string') {
				...
				if (buf.length < end)
					throw new Error('write() failed - not enough space');
				buf.write(v, pos);
			} else {
				end = pos + v.length;
				if (buf.length < end)
